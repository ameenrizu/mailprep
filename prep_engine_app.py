...
class MailPrepLogic:
    ...
    @staticmethod
    def quick_path_signature(path: str) -> Optional[Tuple[int, int]]:
        """
        Lightweight signature for periodic auto-refresh checks.
        Avoids full recursive walk on the UI thread.
        """
        try:
            st = os.stat(path)
...
class MailPrepWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.generated_plain_text_copy = ""

        self.last_preview_signature = None
        self.last_quick_signature = None
    ...
    def _build_action_row(self) -> QHBoxLayout:
        ...
        self.export_csv_btn = QPushButton("\U0001f4e4 Export Final CSV")
        self.export_csv_btn.setObjectName("exportCsvButton")
        self.export_csv_btn.setToolTip(
            "Export filled submission CSV using attached template and current form data"
        )
        self.export_csv_btn.clicked.connect(self.export_submission_csv)
    ...
    def _render_html_only(self, result: BuildResult):
        ...
        clean_lines = self._lines_without_missing(result.lines)
        clean_result = MailPrepLogic.build_result_from_lines(
            clean_lines, self.current_theme, result.signature
        )
        self.generated_clipboard_html = clean_result.clipboard_html
        self.generated_plain_text_copy = clean_result.plain_text
    ...
    def _compose_copy_body_plain_text(self, rows: List[Tuple[str, str]]) -> str:
        ...
        blocks.append(self.generated_plain_text_copy or self.generated_plain_text)
    ...
    def _compose_full_mail_html(self) -> str:
        if not (self.generated_plain_text_copy or self.generated_plain_text):
            return ""
    ...
    def _lines_without_missing(self, lines: List[LineItem]) -> List[LineItem]:
        """Used for copy/export outputs so QC-only missing-frame rows do not leak."""
        return [item for item in lines if item.item_type != "missing"]
    ...
    def render_preview(self, result: BuildResult):
        ...
        if self.current_input_mode == "path":
            ...
            if path:
                self._add_recent_path(path)
                self.last_quick_signature = MailPrepLogic.quick_path_signature(path)
    ...
    def _do_metadata_update(self):
        ...
        self.generated_copy_html = self._compose_copy_rich_html(self.generated_clipboard_html)
        self.generated_copy_plain_text = self._compose_copy_plain_text(
            self.generated_plain_text_copy or self.generated_plain_text
        )
        self.generated_notes_only_html = self._compose_notes_only_html()
        self.generated_notes_only_plain_text = self._compose_notes_only_plain_text()
        self.generated_full_mail_html = self._compose_full_mail_html()
    ...
    def auto_refresh_tick(self):
        ...
        if (
            ...
            current_sig = MailPrepLogic.quick_path_signature(path)
            if current_sig != self.last_quick_signature:
                self.start_preview_job(path, auto=True)
    ...
    def attach_template_sheet(self):
        ...
        if self.current_shot_names:
            reply = QMessageBox.question(
                self,
                "Auto Fill Sheet",
                "Preview data is ready. Export the filled sheet now?",
                QMessageBox.Yes | QMessageBox.No,
            )
            if reply == QMessageBox.Yes:
                self.export_submission_csv()
    ...
    def show_warnings(self):
        try:
            ...
            lines = ["The following issues were found:\n"]
            for idx, warning in enumerate(warnings, start=1):
                lines.append(f"  {idx}. {warning}")

            if self.current_suspicious_names:
                lines.append("\nSuspicious names detected:")
                for name in self.current_suspicious_names:
                    lines.append(f"   - {name}")

            if self.current_duplicate_shot_names:
                lines.append("\nDuplicate shot names:")
                for name in self.current_duplicate_shot_names:
                    lines.append(f"   - {name}")

            if self.current_missing_summary_count:
                lines.append(
                    "\nMissing frame warnings are preview-only and excluded from copy outputs."
                )
